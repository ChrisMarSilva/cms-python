...
heightImg = 450
widthImg = 450
model = utlis.intializePredectionModel()  # LOAD THE CNN MODEL
########################################################################
...
if biggest.size != 0:
    ...
    boxes = utlis.splitBoxes(imgWarpColored)
    print(len(boxes))
    # cv2.imshow("Sample",boxes[65])
    numbers = utlis.getPredection(boxes, model)
    print(numbers)
    imgDetectedDigits = utlis.displayNumbers(imgDetectedDigits, numbers, color=(255, 0, 255))
    ...
    imgSolvedDigits= utlis.displayNumbers(imgSolvedDigits,solvedNumbers)
    ...
    imgDetectedDigits = utlis.drawGrid(imgDetectedDigits)
    imgSolvedDigits = utlis.drawGrid(imgSolvedDigits)
