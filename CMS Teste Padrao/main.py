...
def config_log(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info(
            f"{func.__name__}.inicio - params: {signature}"
        )  # if signature: logger.debug(f"function {func.__name__} called with args {signature}")
        try:
            start_time = (
                time.perf_counter()
            ...
            logger.info(
                f"{func.__name__}.fim - Done in {end_time:.2f}s - {dt.timedelta(seconds=end_time)}"
            )
...
def main():
    start_time = (
        time.perf_counter()
    )  # time.time()  # time.perf_counter()  # time.perf_counter_ns()  # time.process_time() # time.monotonic()
    logger.info(f"Inicio")
    try:
