...
def teste_01_pathlib():
    try:
        ...
        shark = pathlib.Path(pathlib.Path.home(), "ocean", "animals", pathlib.Path("fish", "shark.txt"))
        logger.info(f'{shark=}')
...
async def teste_04_aiofile():
    try:
        ...
        from aiofile import AIOFile, Reader, LineReader, Writer
        import io
        ...
        async with async_open(arguments.source, "rb") as src, async_open(arguments.dest, "wb") as dest:
        ...
        class AsyncDictReader:
            def __init__(self, afp, **kwargs):
                ...
                self.file_reader = LineReader(afp, line_sep=kwargs.pop('line_sep', '\n'),  chunk_size=kwargs.pop('chunk_size', 4096), offset=kwargs.pop('offset', 0))
                self.reader = DictReader(io.TextIOWrapper(self.buffer, encoding=kwargs.pop('encoding', 'utf-8'), errors=kwargs.pop('errors', 'replace')), **kwargs)
                self.line_num = 0
...
async def teste_06_aiopath():
    try:
        ...
        urls = ['https://example.com', 'https://github.com/alexdelorenzo/aiopath', 'https://alexdelorenzo.dev', 'https://dupebot.firstbyte.dev']
